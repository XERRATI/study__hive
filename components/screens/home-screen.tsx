...
/**
 * HOME SCREEN — the exact layout from the original page.tsx, kept
 * pixel-identical. Only the interactive parts are now WIRED:
 *  · weather chip taps open a city picker
...
import { dateKey, QUOTES, weakestSubject } from '@/lib/storage'
...
export function HomeScreen() {
  ...
  const today = dateKey()
  const todayMin = data.dailyLog[today] || 0
  ...
  /* greeting line */
  const name = (data.name || 'Bee').trim()
  const subject = weak || 'your next subject'
  const greeting = `${greetingFor(new Date().getHours())}${data.name ? ` - ${name}` : ''} shall we begin with a study block for ${subject}?`
  ...
  return (
    <div className="relative z-10 mx-3 mt-3 overflow-hidden rounded-[2.25rem] bg-card pb-8 shadow-[0_10px_30px_rgba(0,0,0,0.05)]">
      ...
      <div className="relative px-7 pt-8">
        {editingCity ? (
          ...
          >
            <input
              ...
              className="rounded-md border-2 border-[#7c3aed] bg-card px-3 py-1.5 text-lg font-semibold text-foreground outline-none"
            />
            <button type="submit" className="text-sm font-semibold text-[#7c3aed] underline underline-offset-4">
              Save city
            </button>
          </form>
        ) : (
          <button
            ...
            className="inline-flex flex-col rounded-md border-2 border-[#7c3aed] px-3 py-1.5 text-left text-lg font-semibold leading-tight text-foreground transition-transform active:scale-95"
          >
        ...
        <img
          src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/bee-flower-SoQwPntdvA0etayElW430GoyZ0tum2.png"
      ...
      <div className="mt-7 px-7">
        <p className="text-xl font-semibold leading-snug text-brown text-pretty">{DEFAULT_QUOTE}</p>
      ...
      {todayMin > 0 && (
        <p className="mt-4 text-center text-sm font-semibold text-brown">
