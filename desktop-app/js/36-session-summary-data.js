...
(function(){
  ...
  function addImportPolish(){
    ...
    if(!$('notesFileInput') && $('photoInput')) $('photoInput').insertAdjacentHTML('afterend','<input type="file" id="notesFileInput" accept=".pdf,.txt,.md,application/pdf,text/plain,text/markdown" style="margin-top:8px;"><div class="import-small">Import .pdf, .txt or .md — PDFs get their text extracted (AI-ready).</div>');
  ...
  function readImportFile(file){
    ...
    r.onload=function(e){ var txt=String(e.target.result||''); if(name.endsWith('.ics')){ if($('icsPaste')) $('icsPaste').value=txt; if($('icsStatus')) $('icsStatus').textContent='Loaded '+file.name+'. Preview or import calendar events.'; } else if(name.endsWith('.csv')){ if($('calendarCsvText')) $('calendarCsvText').value=txt; if($('icsStatus')) $('icsStatus').textContent='Loaded '+file.name+'. Press Import CSV/timetable.'; } else if(name.endsWith('.pdf')){ extractPdfText(file).then(function(pt){ if($('photoText')) $('photoText').value=pt; if($('aiImportTextarea')) $('aiImportTextarea').value=pt; if($('photoStatus')) $('photoStatus').textContent='PDF text extracted ('+pt.length+' chars) — make cards, save to Notes, or press 🤖 Generate Flashcards for AI cards.'; if(typeof window.showMilestoneToast==='function') window.showMilestoneToast('📄 PDF loaded — AI-ready', 3500); updateSubjectChips(); }).catch(function(err){ if($('photoStatus')) $('photoStatus').textContent='Could not read that PDF'+(err&&err.message==='pdfjs-cdn-failed'?' (the PDF reader needs internet once)':'')+'. Try a .txt export of the document.'; }); } else { if($('photoText')) $('photoText').value=txt; if($('photoStatus')) $('photoStatus').textContent='Loaded text notes from '+file.name+'. You can save to Notes or make cards.'; updateSubjectChips(); } };
    r.readAsText(file);
  }
  /* ---------- PDF -> text (Turbo-style: PDF in, AI-ready notes out) ----------
     Lazy-loads pdf.js from a CDN (no build step — same pattern as the
     Puter script). Extraction only; the AI work rides the app's existing
     free providers (🤖 Generate Flashcards, coach quiz, notes). */
  ...
  function loadPdfJs(){
    ...
    _pdfjsPromise=new Promise(function(resolve,reject){
      ...
      s.onerror=function(){ _pdfjsPromise=null; reject(new Error('pdfjs-cdn-failed')); };
      document.head.appendChild(s);
    });
    return _pdfjsPromise;
  }
  function extractPdfText(file, maxPages, maxChars){
    maxPages=maxPages||15; maxChars=maxChars||30000;
    return loadPdfJs().then(function(pdfjs){
      return new Promise(function(resolve,reject){
        ...
        fr.onload=function(){
          ...
          pdfjs.getDocument({data:data}).promise.then(function(doc){
            var pages=Math.min(doc.numPages,maxPages), parts=[], chain=Promise.resolve();
            ...
            chain.then(function(){
              ...
              if(!text) { reject(new Error('empty-pdf')); return; }
              resolve(text);
            });
          }).catch(function(){ reject(new Error('pdf-parse-failed')); });
        };
        fr.readAsArrayBuffer(file);
      });
    });
  }
  window.__pdfExtract=extractPdfText;
